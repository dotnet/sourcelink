...
{
    /// <summary>
    /// An <see cref="IBuildEngine4"/> variant of <see cref="MockEngine"/>. Required by tasks that call the
    /// <see cref="IBuildEngine4"/> surface (e.g. <c>RegisterTaskObject</c>/<c>GetRegisteredTaskObject</c>);
    /// the plain <see cref="MockEngine"/> only implements <see cref="IBuildEngine"/>, so passing it to such a
    /// task fails the <c>(IBuildEngine4)BuildEngine</c> cast. The registered-task-object store is a simple
    /// in-memory dictionary scoped to this engine instance.
    /// </summary>
    public sealed class MockEngine4 : IBuildEngine4
    {
